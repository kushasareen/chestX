...
The model achieves and AUC of 0.769 and an F1 score of 0.412, outperforming the average clinical radiologist but still underperforming the ML research titans (See Stanford's 2017 ChestXNet paper for some interesting AI-docor comparison). I spent some time playing around with the model with my brother (who's a medical resident), first getting him to diagnose an x-ray and then running it through the model. It was interesting to see the types of things that my brother would sometimes miss and the model would pick up on and what my model wouldn't catch that my brother would get.
